...
class SideButtons extends React.Component {
  ...
  renderChildButtons() {
    ...
    // To decide which target style should be SideButtonslied function uses css 'scale' property
    ...
    let calculateStylesForNextFrame = prevFrameStyles => {
      ...
      let nextFrameTargetStyles = prevFrameStyles.map(
        (buttonStyleInPreviousFrame, i) => {
          ...
          const shouldSideButtonslyTargetStyle = () => {
            if (isOpen) {
          ...
          return shouldSideButtonslyTargetStyle()
          ...
          <div>
            {interpolatedStyles.map(
              ({ height, left, rotate, scale, top, width }, index) => (
                ...
                >
                  <i
                    className={"fa fa-" + childButtonIcons[index] + " fa-lg"}
                  />
                </div>
              )
            )}
